...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser.add_argument(
        ...
        metavar="NAME",
        help=(
            "Update against this branch instead of the current branch's configured upstream "
            "(falling back to main). If the local checkout is on a different branch, "
            "hermes will switch to the requested branch first (auto-stashing any "
            "uncommitted changes)."
        ),
    )
    update_parser.add_argument(
